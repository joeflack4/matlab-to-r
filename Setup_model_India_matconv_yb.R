source('get_addresses_matconv_yb.R')
...
gps.age      <- c('ch','ad','el')
gps.quar     <- c('q0','q1')
gps.severity <- c('mld','svr')

states1 <- c('S','E','A','P','R') # Stratified by age, quar status
states2 <- c('I','Dx1','Dx2') # Stratified by age, quar status, severity
states3 <- c('H','D') # Stratified by age
states4 <- c('sU','sDx1','sDx2','sQ') # Not stratified

# states1      = {'S','E','A','P','QS','QE','QA','QP','R','H','D'};
# states2      = {'I','Dx1','Dx2','QI'};

i, s, d, lim = get_addresses(c(states1, gps.quar, gps.age), '', '', '', 0)
i, s, d, lim = get_addresses(c(states2, gps.quar, gps.age, gps.severity), i, s, d, lim)
get_addresses(c(states3, gps.age), i, s, d, lim)
nIstates <- nstates
get_addresses({states4}, i, s, d, lim)
d <- as.character(d)
